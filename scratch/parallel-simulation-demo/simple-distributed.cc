...
main(int argc, char* argv[])
{
    ...
    if (verbose)
    {
        ...
        LogComponentEnable("BulkSendApplication",
                           (LogLevel)(LOG_LEVEL_INFO | LOG_PREFIX_NODE | LOG_PREFIX_TIME));
    }
    ...
    // Some default values - Modified to send 5MB per sender
    // Config::SetDefault("ns3::OnOffApplication::PacketSize", UintegerValue(100*1024));  // 1KB packets for better performance
    // Config::SetDefault("ns3::OnOffApplication::DataRate", StringValue("5Mbps"));
    // Config::SetDefault("ns3::OnOffApplication::MaxBytes", UintegerValue(5 * 1024 * 1024));  // 5MB per sender
    ...
    if (systemId == 1)
    {
        ...
        sinkApp.Stop(Seconds(100));  // Increased to match simulation time
    }
    ...
    if (systemId == 0)
    {
        ...
        clientHelper.SetAttribute("MaxBytes", UintegerValue(100 * 1024));  // 100KB per sender
