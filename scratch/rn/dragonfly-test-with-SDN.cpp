...
main(int argc, char** argv)
{
    ...
    // openflow use CsmaHelper
    ns3::CsmaHelper csmaHelper;
    csmaHelper.SetChannelAttribute("DataRate", ns3::StringValue(bandwidth));
    csmaHelper.SetChannelAttribute("Delay", ns3::StringValue(delay));
    ...
    for (SwitchID switch_index = 0; switch_index < switchCount; ++switch_index)
    {
        ...
        for (ServerID server_index = 0; server_index < p; ++server_index)
        {
            ...
            auto devices = csmaHelper.Install(NodeContainer(tor, server));
    ...
    for (GroupID group_index = 0; group_index < g; ++group_index)
    {
        for (SwitchID switch_index1 = 0; switch_index1 < a; ++switch_index1)
        {
            ...
            for (SwitchID switch_index2 = switch_index1 + 1; switch_index2 < a; ++switch_index2)
            {
                ...
                auto devices = csmaHelper.Install(NodeContainer(tor1, tor2));
    ...
    // }
    // install all switches with controller
    for(auto i = 0; i < (int)dragonfly.switches.GetN(); i++){
        auto switchNode = dragonfly.switches.Get(i);
        auto deviceNumber = switchNode->GetNDevices();
        // get all devices
        NetDeviceContainer switchDevices;
        for (uint32_t i = 0; i < deviceNumber; i++)
        {
            switchDevices.Add(switchNode->GetDevice(i));
        }
    ...
    for (OCSLayerID layer = 0; layer < ocs; layer++)
    {
        dragonfly.acticate_one_level(layer);
    }
