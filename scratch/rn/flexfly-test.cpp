...
#include "topology/flexfly-topology.h"
#include "utility.h"
...
main(int argc, char** argv)
{
    ...
    int optimizedGroupPairs = 1;
    uint32_t maxBytes = 1000000000;
    int optimizedLinkPerPair = 2;
    ...
    cmd.AddValue("optimized", "Flexfly: optimized GroupPairs", optimizedGroupPairs);
    cmd.AddValue("max_bytes", "max bytes", maxBytes);
    cmd.AddValue("optimized_link", "Flexfly: optimized LinkPerPair", optimizedLinkPerPair);
    ...
    flexfly.switches.Create(switchCount);
    flexfly.servers.Create(serverCount);
    flexfly.allNodes.Add(flexfly.switches);
    flexfly.allNodes.Add(flexfly.servers);

    if (ugal && ecmp)
    {
        auto listRoutinghelper = Ipv4ListRoutingHelper();
        listRoutinghelper.Add(*flexfly.ugalRoutingHelperForECMPUgal, 0);
        listRoutinghelper.Add(*flexfly.routeMonitorHelper, 1);
        flexfly.internetStackHelper.SetRoutingHelper(listRoutinghelper);
        // congestion monitor should not be with inter-group-ecmp+ugal
    }
    else if (ugal)
    {
        auto listRoutinghelper = Ipv4ListRoutingHelper();
        listRoutinghelper.Add(*flexfly.ugalRoutingHelper, 0);
        listRoutinghelper.Add(*flexfly.routeMonitorHelper, 1);
        flexfly.internetStackHelper.SetRoutingHelper(listRoutinghelper);
        // congestion monitor should not be with inter-group-ecmp+ugal
        flexfly.congestionMonitorApplications =
            flexfly.ugalRoutingHelper->InstallCongestionMonitorApplication(
                flexfly.allNodes,
    ...
    {
        auto listRoutinghelper = Ipv4ListRoutingHelper();
        listRoutinghelper.Add(flexfly.globalRoutingHelper, 0);
        listRoutinghelper.Add(*flexfly.routeMonitorHelper, 1);
        flexfly.internetStackHelper.SetRoutingHelper(listRoutinghelper);
    }
    flexfly.internetStackHelper.Install(flexfly.allNodes);
    ...
    {
        auto tor = flexfly.switches.Get(switch_index);
        for (ServerID server_index = 0; server_index < p; ++server_index)
        {
            auto server = flexfly.servers.Get(switch_index * p + server_index);
            ...
            flexfly.serverMap[server->GetId()] = {serverDevice, serverAddress};
            flexfly.backgroundLinkMap[tor->GetId()][server->GetId()] = {torDevice,
                                                                        serverDevice,
                                                                        torIpv4,
                                                                        serverIpv4,
                                                                        torAddress,
            ...
            flexfly.backgroundLinkMap[server->GetId()][tor->GetId()] = {serverDevice,
                                                                        torDevice,
                                                                        serverIpv4,
                                                                        torIpv4,
                                                                        serverAddress,
                                                                        ...
                                                                        serverIpv4L3Protocol,
                                                                        torIpv4L3Protocol};
            flexfly.linkDeviceState[torDevice][serverDevice] = true;
            flexfly.linkDeviceState[serverDevice][torDevice] = true;
    ...
    for (GroupID group_index = 0; group_index < g; ++group_index)
    {
        ...
        {
            auto tor1 = flexfly.switches.Get(group_index * a + switch_index1);
            for (SwitchID switch_index2 = switch_index1 + 1; switch_index2 < a; ++switch_index2)
            {
                auto tor2 = flexfly.switches.Get(group_index * a + switch_index2);
                ...
                flexfly.backgroundLinkMap[tor1->GetId()][tor2->GetId()] = {tor1Device,
                                                                           tor2Device,
                                                                           tor1Ipv4,
                                                                           tor2Ipv4,
                                                                           tor1Address,
                                                                           ...
                                                                           tor1Ipv4L3Protocol,
                                                                           tor2Ipv4L3Protocol};
                flexfly.backgroundLinkMap[tor2->GetId()][tor1->GetId()] = {tor2Device,
                                                                           tor1Device,
                                                                           tor2Ipv4,
                                                                           tor1Ipv4,
                                                                           tor2Address,
                                                                           ...
                                                                           tor2Ipv4L3Protocol,
                                                                           tor1Ipv4L3Protocol};
                flexfly.linkDeviceState[tor1Device][tor2Device] = true;
                flexfly.linkDeviceState[tor2Device][tor1Device] = true;
                ipv4AddressHelper.NewNetwork();
            }
        }
    }

    // all links between groups
    for (SwitchID server_index1 = 0; server_index1 < switchCount; ++server_index1)
    {
        for (SwitchID server_index2 = server_index1 + 1; server_index2 < switchCount;
             ++server_index2)
        {
            auto tor1 = flexfly.switches.Get(server_index1);
            auto tor2 = flexfly.switches.Get(server_index2);
            if (flexfly.backgroundLinkMap.find(tor1->GetId()) != flexfly.backgroundLinkMap.end() &&
                flexfly.backgroundLinkMap[tor1->GetId()].find(tor2->GetId()) !=
                    flexfly.backgroundLinkMap[tor1->GetId()].end())
            {
                // std::cout << "tor1 = " << tor1->GetId() << " tor2 = " << tor2->GetId() <<
                // std::endl;
                continue;
            }

            if (flexfly.backgroundLinkMap.find(tor2->GetId()) != flexfly.backgroundLinkMap.end() &&
                flexfly.backgroundLinkMap[tor2->GetId()].find(tor1->GetId()) !=
                    flexfly.backgroundLinkMap[tor2->GetId()].end())
            {
                // std::cout << "tor1 = " << tor1->GetId() << " tor2 = " << tor2->GetId() <<
                // std::endl;
                continue;
            ...
            flexfly.linkDeviceState[tor1Device][tor2Device] = false;
            flexfly.linkDeviceState[tor2Device][tor1Device] = false;

            ipv4AddressHelper.NewNetwork();
        }
    }

    // activate links between groups
    for (GroupID group_index1 = 0; group_index1 < g; ++group_index1)
    {
        for (SwitchID switch_index1 = 0; switch_index1 < a; ++switch_index1)
        {
            auto tor1 = flexfly.switches.Get(group_index1 * a + switch_index1);
            for (LinkID link_index = 0; link_index < h; ++link_index)
            {
                ...
                auto tor2 = flexfly.switches.Get(group_index2 * a + switch_index2);
                auto tor1Ipv4 = tor1->GetObject<ns3::Ipv4>();
                auto tor2Ipv4 = tor2->GetObject<ns3::Ipv4>();
                auto tor1Device = flexfly.backgroundLinkMap[tor1->GetId()][tor2->GetId()].device1;
                auto tor2Device = flexfly.backgroundLinkMap[tor1->GetId()][tor2->GetId()].device2;
    ...
    // for (auto iterator = flexfly.servers.Begin(); iterator < flexfly.servers.End(); ++iterator)
    // {
    //     auto node = *iterator;
    //     NS_ASSERT_MSG(
    //         node->GetNDevices() == 2,
    ...
    // for (auto iterator = flexfly.switches.Begin(); iterator < flexfly.switches.End();
    //      ++iterator)
    // {
    //     auto node = *iterator;
    //     NS_ASSERT_MSG(
    //         node->GetNDevices() == p + a + h,
    //         "network devices of tor are not installed correctly"); // with an additional
    //                                                                // loopback device
    // }

    // NS_ASSERT_MSG(flexfly.serverMap.size() == serverCount, "conflict server ports");
    // auto backgroundLinkCountRange =
    //     flexfly.backgroundLinkMap | std::views::values |
    ...
    for (OCSLayerID layer = 0; layer < ocs; ++layer)
    {
        ...
        for (GroupID group_index1 = 0; group_index1 < g; ++group_index1)
        {
            ...
            {
                auto tor1 = flexfly.switches.Get(group_index1 * a + switch_index);
                auto tor2 = flexfly.switches.Get(group_index2 * a + switch_index);
                auto devices = p2pHelper.Install(tor1, tor2);
                auto addresses = ipv4AddressHelper.Assign(devices);
                auto [tor1Device, tor2Device] = ContainerPattern<2>(devices);
                auto tor1Ipv4 = tor1->GetObject<ns3::Ipv4>();
                ...
                auto tor1Ipv4L3Protocol = tor1->GetObject<ns3::Ipv4L3Protocol>();
                auto tor2Ipv4L3Protocol = tor2->GetObject<ns3::Ipv4L3Protocol>();
                flexfly.reconfigurableLinkMap[layer][group_index1][group_index2] = {
                    tor1Device,
                    tor2Device,
                    tor1Ipv4,
                    tor2Ipv4,
                    ...
                    tor1Ipv4L3Protocol,
                    tor2Ipv4L3Protocol};
                flexfly.reconfigurableLinkMap[layer][group_index2][group_index1] = {
                    tor2Device,
                    tor1Device,
                    tor2Ipv4,
                    tor1Ipv4,
    ...
    auto reconfigurableLinkCountRange =
        flexfly.reconfigurableLinkMap | std::views::values | std::views::join | std::views::values |
    ...
    for (GroupID i = 0; i < g; i++)
    {
        for (LinkID j = 0; j < g; j++)
        {
            if (i != j)
            {
                flexfly.groupLinkNumberMap[i][j] = 1;
            }
        }
    }
    ...
    for (uint32_t i = 0; i < g * a * p; i++)
    {
        ...
        flexfly.neighbor_end_pair.push_back({i, (uint32_t)j});
    }
    ...
    for (uint32_t i = 0; i < g * a * p; i++)
    {
        ...
        flexfly.adverse_end_pair.push_back({i, (uint32_t)j});
    }

    if (is_adversial)
    {
        cout << "is_adversial" << endl;
        flexfly.traffic_pattern_end_pair = flexfly.adverse_end_pair;
    }
    else
    {
        cout << "is_neighbor" << endl;
        flexfly.traffic_pattern_end_pair = flexfly.neighbor_end_pair;
    }
    ...
    // add server
    for (uint32_t i = 0; i < flexfly.traffic_pattern_end_pair.size(); i++)
    {
        // std::cout << traffic_pattern_end_pair[i][0] << " " << traffic_pattern_end_pair[i][1] <<
        // endl;
        auto app = sinkHelper.Install(flexfly.servers.Get(flexfly.traffic_pattern_end_pair[i][0]));
    ...
    for (uint32_t i = 0; i < flexfly.traffic_pattern_end_pair.size(); i++)
    {
        ...
        auto peer = flexfly.servers.Get(flexfly.traffic_pattern_end_pair[i][0])->GetId();
        remoteAddress = ns3::AddressValue(
            ns3::InetSocketAddress(flexfly.serverMap[peer].address, port)); // server ip and port
        clientHelper.SetAttribute("Remote", remoteAddress);
        // std::cout <<"b "<< traffic_pattern_end_pair[i][1] << " " <<
        // end_interfaces[traffic_pattern_end_pair[i][1]].GetAddress(1) << endl;
        auto clientApp =
            clientHelper.Install((flexfly.servers.Get(flexfly.traffic_pattern_end_pair[i][1])));
        auto address =
            flexfly.serverMap[flexfly.servers.Get(flexfly.traffic_pattern_end_pair[i][1])->GetId()]
                .address;
        flexfly.address2Onoffapplication[address] = DynamicCast<OnOffApplication>(clientApp.Get(0));
    ...
    flexfly.congestionMonitorApplications.Stop(ns3::Seconds(stop_time));
    ...
    // NS_ASSERT_MSG(traces.size() <= serverCount, "flexfly size is not enough");

    // for (ns3::MPIRankIDType rankID = 0; rankID < traces.size(); ++rankID) {
    //     auto node = flexfly.servers.Get(rankID);
    //     auto ip = flexfly.serverMap[node->GetId()].address;
    ...
    std::cout << "start flexfly simulation " << optimizedGroupPairs << " " << optimizedLinkPerPair
              << std::endl;
    flexfly.optimized_links(optimizedGroupPairs, optimizedLinkPerPair);
    std::cout << "finish flexfly simulation" << std::endl;

    for (auto i = 0; i < (int)switchCount; ++i)
    {
        flexfly.switchID2Index[flexfly.switches.Get(i)->GetId()] = i;
    }
    // for (OCSLayerID layer = 0; layer < ocs; layer++)
    // {
    //     flexfly.activate_one_level(layer);
    // }
    ...
    if (ugal && ecmp)
    {
        flexfly.ugalRoutingHelperForECMPUgal->NotifyLinkChanges();
    }
    else if (ugal)
    {
        flexfly.ugalRoutingHelper->NotifyLinkChanges();
    }

    flexfly.congestionMonitorApplications.Start(ns3::Seconds(start_time));
    // applications.Start(ns3::Seconds(0));
    Simulator::Stop(Seconds(stop_time + 10));
    std::cout << "Flexfly Run..." << endl;
    ns3::Simulator::Run();
    std::cout << "Run done." << endl;
    std::cout << "totalHops: " << flexfly.totalHops << std::endl;
