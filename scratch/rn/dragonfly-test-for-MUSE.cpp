...
main(int argc, char** argv)
{
    ...
    for (SwitchID server_index1 = 0; server_index1 < switchCount; ++server_index1)
    {
        for (SwitchID server_index2 = server_index1 + 1; server_index2 < switchCount;
             ++server_index2)
        {
            auto tor1 = dragonfly.switches.Get(server_index1);
            auto tor2 = dragonfly.switches.Get(server_index2);
            if (dragonfly.backgroundLinkMap.find(tor1->GetId()) !=
                    dragonfly.backgroundLinkMap.end() &&
                dragonfly.backgroundLinkMap[tor1->GetId()].find(tor2->GetId()) !=
                    dragonfly.backgroundLinkMap[tor1->GetId()].end())
            {
            ...
            if (dragonfly.backgroundLinkMap.find(tor2->GetId()) !=
                    dragonfly.backgroundLinkMap.end() &&
                dragonfly.backgroundLinkMap[tor2->GetId()].find(tor1->GetId()) !=
                    dragonfly.backgroundLinkMap[tor2->GetId()].end())
            {
                // std::cout << "tor1 = " << tor1->GetId() << " tor2 = " << tor2->GetId() <<
                // std::endl;
                continue;
    ...
    for (GroupID group_index1 = 0; group_index1 < g; ++group_index1)
    {
        for (SwitchID switch_index1 = 0; switch_index1 < a; ++switch_index1)
        {
            ...
            for (LinkID link_index = 0; link_index < h; ++link_index)
            {
                ...
                auto tor1Ipv4 = tor1->GetObject<ns3::Ipv4>();
                auto tor2Ipv4 = tor2->GetObject<ns3::Ipv4>();
                auto tor1Device = dragonfly.backgroundLinkMap[tor1->GetId()][tor2->GetId()].device1;
                auto tor2Device = dragonfly.backgroundLinkMap[tor1->GetId()][tor2->GetId()].device2;
