...
main(int argc, char** argv)
{
    ...
    {
        // server-tor links
        for (SwitchID switch_index = 0; switch_index < fattree.number_of_edge_switches;
             ++switch_index)
        {
            auto tor = fattree.edge_switches.Get(switch_index);
            for (ServerID server_index = 0;
                 server_index < fattree.number_of_servers_per_edge_switch;
                 ++server_index)
            {
        ...
        {
            for (SwitchID switch_index1 = 0;
                 switch_index1 < fattree.number_of_edge_switches_per_pod;
                 ++switch_index1)
    ...
    NS_ASSERT_MSG(traces.size() <= fattree.number_of_servers, "fattree size is not enough");
    ...
    {
        trace.emplace([&remaining, &fattree](auto&) -> ns3::MPIOperation<void> {
            --remaining;
            if (remaining <= 0)
            {
                ns3::Simulator::Stop(ns3::Now() + Seconds(1000));
            }
            co_return;
        });
    ...
    std::cout << "程序运行时间为: " << duration.count() << " ns" << std::endl;
    auto simulation_duration = simulation_end - simulation_start;
    std::cout << "程序运行时间为: " << simulation_duration << " 秒" << std::endl;
