...
namespace ns3 {
    NS_LOG_COMPONENT_DEFINE("Rip-CoDES");
    NS_OBJECT_ENSURE_REGISTERED(Rip);

    RipRoutingTableEntry::RipRoutingTableEntry(): m_tag(0), m_metric(0), m_status(RIP_INVALID), m_changed(false) {}
    ...
    Rip::Rip(): m_ipv4(nullptr), m_splitHorizonStrategy(Rip::POISON_REVERSE), m_initialized(false) {
        std::cout << "Rip::Rip() rip-CoDES.cpp" << std::endl;
    ...
    std::set<uint32_t> Rip::GetInterfaceExclusions() const { return m_interfaceExclusions; }
    ...
    void Rip::NotifyRemoveAddress(uint32_t interface, Ipv4InterfaceAddress address) {
        ...
        for (auto it = m_routes.begin(); it != m_routes.end(); it++) {
            if (it->first->GetInterface() == interface && it->first->IsNetwork() &&
                it->first->GetDestNetwork() == networkAddress && it->first->GetDestNetworkMask() == networkMask) {
                InvalidateRoute(it->first);
            }
        }
    ...
    void Rip::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const {
        ...
        *os << "Node: " << m_ipv4->GetObject<Node>()->GetId() << ", Time: " << Now().As(unit)
            << ", Local time: " << m_ipv4->GetObject<Node>()->GetLocalTime().As(unit) << ", IPv4 RIP table" << std::endl;
    ...
    void Rip::DoInitialize() {
        ...
        std::cout << "Rip::DoInitialize() unicast" << std::endl;
        ...
        std::cout << "Rip::DoInitialize() rip-CoDES.cpp" << std::endl;
    }
        ...
        for (auto iter = m_unicastSocketList.begin(); iter != m_unicastSocketList.end(); iter++) {
            ...
            if (m_interfaceExclusions.find(interface) == m_interfaceExclusions.end()) {
                ...
                for (auto rtIter = m_routes.begin(); rtIter != m_routes.end(); rtIter++) {
                    ...
                    Ipv4InterfaceAddress rtDestAddr = Ipv4InterfaceAddress(rtIter->first->GetDestNetwork(),
                    ...
                    if ((isGlobal || isDefaultRoute) && (periodic || rtIter->first->IsRouteChanged()) && !sameNetwork) {
                        RipRte rte;
        ...
        if (rtes.size() == 1) {
            if (rtes.begin()->GetPrefix() == Ipv4Address::GetAny() &&
                ...
                if (m_interfaceExclusions.find(incomingInterface) == m_interfaceExclusions.end()) {
                    // we use one of the sending sockets of the request incoming interface, as they're bound to the right interface
                    ...
                    for (auto rtIter = m_routes.begin(); rtIter != m_routes.end(); rtIter++) {
                        ...
                        if ((isGlobal || isDefaultRoute) && (rtIter->first->GetRouteStatus() == RipRoutingTableEntry::RIP_VALID)) {
                            ...
                            rte.SetRouteTag(rtIter->first->GetRouteTag());
                            if ((m_splitHorizonStrategy != SPLIT_HORIZON) || (m_splitHorizonStrategy == SPLIT_HORIZON && !splitHorizoning)) {
                                hdr.AddRte(rte);
                            }
                        }
                        if (hdr.GetRteNumber() == maxRte) {
    ...
    void Rip::HandleResponses(RipHeader hdr, Ipv4Address senderAddress, uint32_t incomingInterface, uint8_t hopLimit) {
        ...
        for (auto iter = rtes.begin(); iter != rtes.end(); iter++) {
            ...
            if (iter->GetPrefix().IsLocalhost() || iter->GetPrefix().IsBroadcast() || iter->GetPrefix().IsMulticast()) {
                ...
                if (it->first->GetDestNetwork() == rteAddr && it->first->GetDestNetworkMask() == rtePrefixMask) {
                    ...
                    if (rteMetric < it->first->GetRouteMetric()) {
                        if (senderAddress != it->first->GetGateway()) {
                            auto route = new RipRoutingTableEntry(rteAddr, rtePrefixMask, senderAddress, incomingInterface);
                            delete it->first;
                            it->first = route;
                        }
    ...
    bool Rip::RouteInput(Ptr<const Packet> p, const Ipv4Header& header, Ptr<const NetDevice> idev,
                const UnicastForwardCallback& ucb, const MulticastForwardCallback& mcb, const LocalDeliverCallback& lcb, const ErrorCallback& ecb) {
        ...
        if (rtentry) {
            NS_LOG_LOGIC("Found unicast destination");
