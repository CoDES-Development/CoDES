...
Ipv4RipTest::Ipv4RipTest(): TestCase("RIP") { std::cout << "Ipv4RipTest" << std::endl; }
...
void Ipv4RipTest::DoSendData(Ptr<Socket> socket, std::string to) {
    Address realTo = InetSocketAddress(Ipv4Address(to.c_str()), 1234);
    NS_TEST_EXPECT_MSG_EQ(socket->SendTo(Create<Packet>(123), 0, realTo), 123, "100");
}

void Ipv4RipTest::SendData(Ptr<Socket> socket, std::string to) {
...
Ipv4RipCountToInfinityTest::Ipv4RipCountToInfinityTest(): TestCase("RIP counting to infinity") {
    std::cout << "Ipv4RipCountToInfinityTest" << std::endl;
}
...
void Ipv4RipCountToInfinityTest::DoSendData(Ptr<Socket> socket, std::string to) {
    Address realTo = InetSocketAddress(Ipv4Address(to.c_str()), 1234);
    NS_TEST_EXPECT_MSG_EQ(socket->SendTo(Create<Packet>(123), 0, realTo), 123, "100");
}

void Ipv4RipCountToInfinityTest::SendData(Ptr<Socket> socket, std::string to) {
    m_receivedPacket = Create<Packet>();
    Simulator::ScheduleWithContext(socket->GetNode()->GetId(),
                                   Seconds(60),
...
Ipv4RipSplitHorizonStrategyTest::Ipv4RipSplitHorizonStrategyTest(Rip::SplitHorizonType_e strategy) : TestCase("RIP Split Horizon strategy") {
    std::cout << "Ipv4RipSplitHorizonStrategyTest" << std::endl;
...
int main() {
    Ipv4RipTest test1;
    Ipv4RipCountToInfinityTest test2;
    Ipv4RipSplitHorizonStrategyTest test3(Rip::SPLIT_HORIZON);
    test1.DoRun();
