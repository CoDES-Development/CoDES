...
void QueueDisc::Pause() {
    m_pause = true;
}

void QueueDisc::Resume() {
    auto was_paused = m_pause;
    m_pause = false;
    if (was_paused) {
        // if the queue was paused, we need to call Run to dequeue packets
        Run();
    }
}

bool QueueDisc::IsPaused() const {
    return m_pause;
}
...
QueueDisc::Enqueue(Ptr<QueueDiscItem> item)
{
    ...
    // if(set_local_draining == 1)
    //     std::cout << "enqueue" << retval <<std::endl;
    return retval;
}
...
QueueDisc::DequeuePacket()
{
    ...
    if (IsPaused()) // if the queue is paused, return an empty packet
    {
        return nullptr;
    }
...
QueueDisc::Transmit(Ptr<QueueDiscItem> item)
{
    ...
    {
        // std::cout<< item << "nTotalSentPackets " << GetStats().nTotalSentPackets << " nTotalEnqueuedPackets "
        // << GetStats().nTotalEnqueuedPackets << " nTotalReceivedPackets " << GetStats().nTotalReceivedPackets
        // << " nTotalDroppedPackets " << GetStats().nTotalDroppedPackets
        // << " nTotalDequeuedPackets " << GetStats().nTotalDequeuedPackets << std::endl;
        Requeue(item);
        return false;
    }
