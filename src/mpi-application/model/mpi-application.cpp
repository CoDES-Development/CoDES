...
ns3::MPIOperation<std::unordered_map<ns3::MPIRankIDType, std::shared_ptr<ns3::MPISocket>>> ns3::MPIApplication::connect(size_t cache_limit, MPIRankIDType rankID, NS3Node node, const std::map<MPIRankIDType, Address> &addresses, const std::map<Address, MPIRankIDType> &ranks) {
    auto &self = addresses.at(rankID);
    MPISocket listener{node, TcpSocketFactory::GetTypeId(), cache_limit};
    listener.bind(self);
    ...
    for (auto &[rank, address]: addresses) {
        if (rank < rankID) {
            operations.push_back(listener.accept().then([&sockets, &ranks](auto d) {
                ...
                sockets[ranks.at(ip)] = std::make_shared<MPISocket>(std::move(s));
            }));
        }
        if (rank > rankID) {
            sockets[rank] = std::make_shared<MPISocket>(node, TcpSocketFactory::GetTypeId(), cache_limit);
...
ns3::MPIOperation<void> ns3::MPIApplication::Initialize(size_t mtu_size) {
    if (status != Status::INITIAL) {
        throw std::runtime_error("MPIApplication::Init() should only be called once");
    }
    auto cache_limit = mtu_size * 100;
    std::unordered_map<MPIRankIDType, std::shared_ptr<MPISocket>> selfSockets;
    std::unordered_map<MPIRankIDType, std::shared_ptr<MPISocket>> worldSockets = std::move(co_await connect(cache_limit, rankID, GetNode(), addresses, ranks));
    worldSockets[rankID] = std::make_shared<MPISocket>(cache_limit); // loopback
    selfSockets[rankID] = std::make_shared<MPISocket>(cache_limit); // loopback
