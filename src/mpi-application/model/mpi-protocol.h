...
namespace ns3 {
    ...
    struct MPIObjectReader<std::vector<T>> {
        MPIOperation<std::vector<T>> operator()(MPISocket &socket) const requires MPIReadable<T> {
            ...
            if constexpr (MPIBatchReadable<T>) {
                ...
                if (error != NS3Error::ERROR_NOTERROR) {
                    throw MPISocketException{"Batch read vector failed, reason: " + format(error)};
                }
        ...
        template<typename ...U>
        MPIOperation<void> operator()(MPISocket &socket, MPIFakePacket p, std::size_t count, U &&...u) const requires MPIFakeReadable<T, U...> {
    ...
    struct MPIObjectWriter<std::vector<T>> {
        MPIOperation<void> operator()(MPISocket &socket, const std::vector<T> &vector) const requires MPIWritable<T> {
            ...
            if constexpr (MPIBatchWritable<T>) {
                ...
                if (error != NS3Error::ERROR_NOTERROR) {
                    throw MPISocketException{"Batch write vector failed, reason: " + format(error)};
                }
            } else {
                for (auto &t: vector) {
        ...
        template<typename ...U>
        MPIOperation<void> operator()(MPISocket &socket, MPIFakePacket p, std::size_t count, U &&...u) const requires MPIFakeWritable<T, U...> {
