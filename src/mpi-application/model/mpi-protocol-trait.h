...
namespace ns3 {
    ...
    concept MPIReadable=requires(MPIObjectReader<T> reader){
        { reader(std::declval<MPISocket &>()) } -> std::same_as<MPIOperation<T>>;
    };
    ...
    concept MPIWritable=requires(MPIObjectWriter<T> writer){
        { writer(std::declval<MPISocket &>(), std::declval<T>()) } -> std::same_as<MPIOperation<void>>;
    };
    ...
    concept MPIFakeReadable=requires(MPIObjectReader<T> reader){
        { reader(std::declval<MPISocket &>(), std::declval<MPIFakePacket>(), std::declval<U>()...) } -> std::same_as<MPIOperation<void>>;
    };
    ...
    concept MPIFakeWritable=requires(MPIObjectWriter<T> writer){
        { writer(std::declval<MPISocket &>(), std::declval<MPIFakePacket>(), std::declval<U>()...) } -> std::same_as<MPIOperation<void>>;
    };
