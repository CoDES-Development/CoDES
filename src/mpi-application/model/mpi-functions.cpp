...
std::queue<ns3::MPIFunction> ns3::parse_trace(std::filesystem::path trace_path) {
    if (trace_path.filename().string().substr(0, 5) != "dumpi" || trace_path.filename().extension() != ".bin") {
        throw std::runtime_error(trace_path.filename().string() + " is not a valid dumpi trace file");
    }
    ...
    while (mpi_function < DUMPI_END_OF_STREAM) {
        ...
        mpi_functions.emplace([mpi_function](auto &) -> MPIOperation<void> {
        ...
        switch (mpi_function) {
            case DUMPI_Init: {
                ...
                mpi_functions.emplace([](MPIApplication &application) -> MPIOperation<void> {
                    std::cout << "Executing MPI_Init..." << std::endl;
            ...
            case DUMPI_Comm_size: {
                ...
                mpi_functions.emplace([comm, size](MPIApplication &application) -> MPIOperation<void> {
                    std::cout << "Executing MPI_Comm_size... Parsed comm: " << comm << ", size: " << size << std::endl;
                ...
                mpi_functions.emplace([interval](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
            ...
            case DUMPI_Comm_rank: {
                ...
                mpi_functions.emplace([comm, rank](MPIApplication &application) -> MPIOperation<void> {
                    std::cout << "Executing MPI_Comm_rank... Parsed comm: " << comm << ", rank: " << rank << std::endl;
            ...
            case DUMPI_Comm_dup: {
                dumpi_comm old_comm = get16(fp);
                dumpi_comm new_comm = get16(fp);

                mpi_functions.emplace([old_comm, new_comm](MPIApplication &application) -> MPIOperation<void> {
                    std::cout << "Executing MPI_Comm_dup... Parsed old_comm: " << old_comm << ", new_comm: " << new_comm << std::endl;
                    application.duplicate_communicator(old_comm, new_comm);
                    if (application.communicator(old_comm).GroupMembers() != application.communicator(new_comm).GroupMembers()) {
                        throw std::runtime_error{"MPI_Comm_dup error"};
                    }
                    co_return;
                });
                int64_t interval = get_interval(cpu.start.sec, cpu.start.nsec, cpu.stop.sec, cpu.stop.nsec);
            ...
            case DUMPI_Comm_free: {
                dumpi_comm target_comm = get16(fp);

                mpi_functions.emplace([target_comm](MPIApplication &application) -> MPIOperation<void> {
                    std::cout << "Executing MPI_Comm_free... Parsed target_comm: " << target_comm << std::endl;
                    application.free_communicator(target_comm);
                    co_return;
                });
                int64_t interval = get_interval(cpu.start.sec, cpu.start.nsec, cpu.stop.sec, cpu.stop.nsec);
                mpi_functions.emplace([interval](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
            ...
            case DUMPI_Irecv: {
                ...
                mpi_functions.emplace([comm, datatype, source, request, count](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Irecv... Parsed count: " << count << ", datatype: " << datatype <<
                    ", source: " << source << ", comm: " << comm << ", request: " << request << std::endl;
                    auto &c = application.communicator(comm);
                    application.requests.at(request) = type_mapping(datatype, [&c, source, count]<typename T>() {
            ...
            case DUMPI_Isend: {
                ...
                mpi_functions.emplace([comm, datatype, dest, request, count](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Isend... Parsed count: " << count << ", datatype: " << datatype <<
                    ", dest: " << dest << ", comm: " << comm << ", request: " << request << std::endl;
                    auto &c = application.communicator(comm);
                    application.requests.at(request) = type_mapping(datatype, [&c, dest, count]<typename T>() {
            ...
            case DUMPI_Send: {
                ...
                mpi_functions.emplace([comm, datatype, dest, count](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Send... Parsed count: " << count << ", datatype: " << datatype <<
                    ", dest: " << dest << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
            ...
            case DUMPI_Recv: {
                ...
                mpi_functions.emplace([comm, datatype, source, count](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Recv... Parsed count: " << count << ", datatype: " << datatype <<
                    ", source: " << source << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
            ...
            case DUMPI_Waitall: {
                ...
                mpi_functions.emplace([requests](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Waitall... Parsed requests: ";
                    ...
                    for (auto request: requests) {
                        if (request != NULL_REQUEST) {
                            co_await application.requests.at(request);
                        }
                    }
                });
                break;
            ...
            case DUMPI_Waitsome: {
                ...
                mpi_functions.emplace([requests](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Waitsome... Parsed requests: ";
                    for (auto request: requests) {
                        std::cout << request << " ";
                    }
                    std::cout << std::endl;
            ...
            case DUMPI_Wait: {
                ...
                mpi_functions.emplace([request](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Wait... Parsed request: " << request << std::endl;
                    co_await application.requests.at(request);
                });
                break;
            }
            case DUMPI_Barrier: {
                ...
                mpi_functions.emplace([comm](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Barrier... Parsed comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
                    co_await c.Barrier();
                });
                break;
            ...
            case DUMPI_Wtime: { //该函数压根没读取二进制数据，可以忽略
                mpi_functions.emplace([](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Wtime..." << std::endl;
                    co_return;
                });
                int64_t interval = get_interval(cpu.start.sec, cpu.start.nsec, cpu.stop.sec, cpu.stop.nsec);
                mpi_functions.emplace([interval](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
            ...
            case DUMPI_Allreduce: {
                ...
                mpi_functions.emplace([comm, datatype, count](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Allreduce... Parsed count: " << count << ", datatype: " << datatype <<
                    ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
            ...
            case DUMPI_Reduce: {
                ...
                mpi_functions.emplace([comm, datatype, count, root](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Reduce... Parsed count: " << count << ", datatype: " << datatype <<
                    ", root: " << root << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
            ...
            case DUMPI_Bcast: {
                ...
                mpi_functions.emplace([comm, datatype, count, root](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Bcast... Parsed count: " << count << ", datatype: " << datatype <<
                    ", root: " << root << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
                    co_await type_mapping(datatype, [&c, count, root]<typename T>() {
            ...
            case DUMPI_Alltoall: {
                ...
                mpi_functions.emplace([comm, send_count, send_type, recv_count, recv_type](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Alltoall... Parsed send_count: " << send_count << ", send_type: " << send_type <<
                    ", recv_count: " << recv_count << ", recv_type: " << recv_type << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
            ...
            case DUMPI_Alltoallv: {
                ...
                mpi_functions.emplace([comm, send_counts, send_type, recv_counts, recv_type](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Alltoallv... Parsed send_counts: ";
                    for (auto send_count: send_counts) {
                        std::cout << send_count << " ";
                    }
                    std::cout << ", send_type: " << send_type << ", recv_counts: ";
                    for (auto recv_count: recv_counts) {
                        std::cout << recv_count << " ";
                    }
                    std::cout << ", recv_type: " << recv_type << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
...
//            }
            case DUMPI_Rsend: {
                ...
                mpi_functions.emplace([comm, datatype, dest, count](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Rsend... Parsed count: " << count << ", datatype: " << datatype <<
                    ", dest: " << dest << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
                    co_await type_mapping(datatype, [&c, dest, count]<typename T>() {
                        return c.template Send<std::vector<T>>(ns3::FakePacket, dest, count);
            ...
            case DUMPI_Allgather: {
                ...
                mpi_functions.emplace([comm, send_count, send_type, recv_count, recv_type](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Allgather... Parsed send_count: " << send_count << ", send_type: " << send_type <<
                    ", recv_count: " << recv_count << ", recv_type: " << recv_type << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
            ...
            case DUMPI_Allgatherv: {
                ...
                mpi_functions.emplace([comm, send_count, send_type, recv_counts, recv_type](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Allgatherv... Parsed send_count: " << send_count;
                    std::cout << ", send_type: " << send_type << ", recv_counts: ";
                    for (auto recv_count: recv_counts) {
                        std::cout << recv_count << " ";
                    }
            ...
            case DUMPI_Gather: {
                ...
                mpi_functions.emplace([comm, root, send_count, send_type](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Gather... Parsed send_count: " << send_count << ", send_type: " << send_type <<
                    ", root: " << root << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
            ...
            case DUMPI_Gatherv: {
                ...
                mpi_functions.emplace([comm, comm_rank, root, send_type, send_count, recv_counts, recv_type](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Gatherv... Parsed send_count: " << send_count << ", send_type: " << send_type <<
                    ", root: " << root << ", comm: " << comm << ", recv_counts: ";
                    for (auto recv_count: recv_counts) {
                        std::cout << recv_count << " ";
                    }
                    std::cout << ", recv_type: " << recv_type << std::endl;
            ...
            case DUMPI_Initialized: {
                int result = get32(fp);
                mpi_functions.emplace([result](MPIApplication &application) -> MPIOperation<void> {
                    std::cout << "Executing MPI_Initialized... Parsed result: " << result << std::endl;
            ...
            case DUMPI_Testall: {
                ...
                mpi_functions.emplace([requests](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Testall... Parsed requests: ";
                    for (auto request: requests) {
                        std::cout << request << " ";
                    }
                    std::cout << std::endl;
            ...
            case DUMPI_Sendrecv: {
                ...
                mpi_functions.emplace([comm, send_count, send_type, dest, recv_count, source, recv_type](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Sendrecv... Parsed send_count: " << send_count << ", send_type: " << send_type << ", dest: " << dest <<
                    ", recv_count: " << recv_count << ", recv_type: " << recv_type << ", source: " << source << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
            ...
            case DUMPI_Sendrecv_replace: {
                ...
                mpi_functions.emplace([comm, count, datatype, dest, source](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Sendrecv_replace... Parsed count: " << count << ", datatype: " << datatype <<
                    ", dest: " << dest << ", source: " << source << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
            ...
            case DUMPI_Scatter: {
                ...
                mpi_functions.emplace([comm, root, recv_count, recv_type](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Scatter... Parsed recv_count: " << recv_count << ", recv_type: " << recv_type <<
                    ", root: " << root << ", comm: " << comm << std::endl;
                    auto &c = application.communicator(comm);
            ...
            case DUMPI_Scatterv: {
                ...
                mpi_functions.emplace([comm, comm_rank, root, send_type, recv_count, recv_type, send_counts](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Scatterv... Parsed send_type: " << send_type << ", recv_count: " << recv_count <<
                    ", recv_type: " << recv_type << ", root: " << root << ", comm: " << comm << ", send_counts: ";
                    for (auto send_count: send_counts) {
                        std::cout << send_count << " ";
                    }
                    std::cout << std::endl;
            ...
            case DUMPI_Reduce_scatter: {
                ...
                mpi_functions.emplace([comm, recv_counts, datatype](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Reduce_scatter... Parsed recv_counts: ";
                    for (auto recv_count: recv_counts) {
                        std::cout << recv_count << " ";
                    }
                    std::cout << ", datatype: " << datatype << ", comm: " << comm << std::endl;
...
//            }
            case DUMPI_Finalize: {
                mpi_functions.emplace([](ns3::MPIApplication &application) -> ns3::MPIOperation<void> {
                    std::cout << "Executing MPI_Finalize..." << std::endl;
