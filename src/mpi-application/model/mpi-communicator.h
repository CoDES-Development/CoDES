...
namespace ns3 {
    ...
    private:
        ...
        std::unordered_map<MPIRankIDType, std::shared_ptr<MPISocket>> sockets;
    ...
    public:
        ...
        MPICommunicator(MPIRankIDType rankID, const std::shared_ptr<std::mt19937> &randomEngine, std::unordered_map<MPIRankIDType, std::shared_ptr<MPISocket>> &&sockets) noexcept;
        ...
        MPIOperation<void> Send(MPIRankIDType rank, T data) {
            logDebug(logName, std::format("{} send data of type {} to rank {}", rankID, typeid(T).name(), rank));
        ...
        MPIOperation<void> Send(MPIFakePacket p, MPIRankIDType rank, U ...u) {
            logDebug(logName, std::format("{} send fake data of type {} to rank {}, fake parameters: {}", rankID, typeid(T).name(), rank, to_string(u...)));
        ...
        MPIOperation<T> Recv(MPIRankIDType rank) {
            logDebug(logName, std::format("{} recv data of type {} from rank {}", rankID, typeid(T).name(), rank));
        ...
        MPIOperation<void> Recv(MPIFakePacket p, MPIRankIDType rank, U ...u) {
            logDebug(logName, std::format("{} recv fake data of type {} from rank {}, fake parameters: {}", rankID, typeid(T).name(), rank, to_string(u...)));
        ...
        MPIOperation<R> SendRecv(MPIRankIDType destination, S data, MPIRankIDType source) {
            logDebug(logName, std::format("{} send data of type {} to rank {} and recv data of type {} from rank {}", rankID, typeid(S).name(), destination, typeid(R).name(), source));
        ...
        MPIOperation<void> SendRecv(MPIFakePacket p, MPIRankIDType destination, MPIRankIDType source, const std::tuple<US...> &uS, const std::tuple<UR...> &uR) {
            logDebug(logName, std::format("{} send fake data of type {} to rank {} and recv fake data of type {} from rank {}, fake parameters S: {}, fake parameters R: {}", rankID, typeid(S).name(), destination, typeid(R).name(), source, to_string(uS), to_string(uR)));
        ...
        MPIOperation<std::unordered_map<MPIRankIDType, T>> Gather(MPIRankIDType root, T data) {
            logDebug(logName, std::format("{} gather data of type {} to rank {}", rankID, typeid(T).name(), root));
        ...
        MPIOperation<void> Gather(MPIFakePacket p, MPIRankIDType root, U ...u) {
            logDebug(logName, std::format("{} gather fake data of type {} to rank {}, fake parameters: ", rankID, typeid(T).name(), root, to_string(u...)));
            std::unordered_map<MPIRankIDType, T> result;
        ...
        MPIOperation<void> Gather(MPIFakePacket p, MPIRankIDType root, const std::unordered_map<MPIRankIDType, std::tuple<U...>> &u) {
            logDebug(logName, std::format("{} gather fake data of type {} to rank {}", rankID, typeid(T).name(), root));
            std::unordered_map<MPIRankIDType, T> result;
        ...
        template<MPIObject T>
        MPIOperation<std::unordered_map<MPIRankIDType, T>> AllGather(T data) {
            logDebug(logName, std::format("{} all gather data of type {}", rankID, typeid(T).name()));
            std::unordered_map<MPIRankIDType, MPIOperation<std::unordered_map<MPIRankIDType, T>>> operations;
            for (auto rank: sockets | std::ranges::views::keys) {
        ...
        MPIOperation<void> AllGather(MPIFakePacket p, U ...u) {
            logDebug(logName, std::format("{} all gather fake data of type {}, fake parameters: {}", rankID, typeid(T).name(), to_string(u...)));
            std::vector<MPIOperation<void>> operations;
            for (auto rank: sockets | std::ranges::views::keys) {
        ...
        MPIOperation<void> AllGather(MPIFakePacket p, const std::unordered_map<MPIRankIDType, std::tuple<U...>> &u) {
            logDebug(logName, std::format("{} all gather fake data of type {}, fake parameters omitted", rankID, typeid(T).name()));
            std::vector<MPIOperation<void>> operations;
            for (auto rank: sockets | std::ranges::views::keys) {
                operations.push_back(Gather<T>(p, rank, u));
            }
        ...
        MPIOperation<T> Scatter(MPIRankIDType root, const std::unordered_map<MPIRankIDType, T> &data) {
            logDebug(logName, std::format("{} scatter data of type {} from rank {}", rankID, typeid(T).name(), root));
            auto o = Recv<T>(root);
            if (rankID == root) {
                std::vector<MPIOperation<void>> operations;
                for (auto rank: sockets | std::ranges::views::keys) {
        ...
        MPIOperation<void> Scatter(MPIFakePacket p, MPIRankIDType root, U ...u) {
            logDebug(logName, std::format("{} scatter fake data of type {} from rank {}, fake parameters: {}", rankID, typeid(T).name(), root, to_string(u...)));
        ...
        MPIOperation<void> Scatter(MPIFakePacket p, MPIRankIDType root, const std::unordered_map<MPIRankIDType, std::tuple<U...>> &u) {
            logDebug(logName, std::format("{} scatter fake data of type {} from rank {}, fake parameters omitted", rankID, typeid(T).name(), root));
        ...
        MPIOperation<T> Broadcast(MPIRankIDType root, const std::optional<T> &data) {
            logDebug(logName, std::format("{} broadcast data of type {} from rank {}", rankID, typeid(T).name(), root));
            auto o = Recv<T>(root);
            if (rankID == root) {
                std::vector<MPIOperation<void>> operations;
                for (auto rank: sockets | std::ranges::views::keys) {
        ...
        MPIOperation<void> Broadcast(MPIFakePacket p, MPIRankIDType root, U ...u) {
            logDebug(logName, std::format("{} broadcast fake data of type {} from rank {}, fake parameters: {}", rankID, typeid(T).name(), root, to_string(u...)));
            auto o = Recv<T>(p, root, u...);
            if (rankID == root) {
                std::vector<MPIOperation<void>> operations;
                for (auto rank: sockets | std::ranges::views::keys) {
        ...
        MPIOperation<std::optional<std::decay_t<T>>> Reduce(MPIRankIDType root, T data, U ...u) {
            logDebug(logName, std::format("{} reduce data of type {} at rank {}, parameters: {}", rankID, typeid(T).name(), root, to_string(u...)));
        ...
        MPIOperation<void> Reduce(MPIFakePacket p, MPIRankIDType root, U ...u) {
            logDebug(logName, std::format("{} reduce fake data of type {} at rank {}, fake parameters: {}", rankID, typeid(T).name(), root, to_string(u...)));
        ...
        MPIOperation<T> ReduceScatter(const std::unordered_map<MPIRankIDType, T> &data, P ...p) {
            logDebug(logName, std::format("{} reduce scatter data of type {}", rankID, typeid(T).name()));
            std::unordered_map<MPIRankIDType, MPIOperation<T>> operations;
        ...
        MPIOperation<void> ReduceScatter(MPIFakePacket p, const std::unordered_map<MPIRankIDType, std::tuple<U...>> &u) {
            logDebug(logName, std::format("{} reduce scatter fake data of type {}, fake parameters omitted", rankID, typeid(T).name()));
            std::vector<MPIOperation<void>> operations;
            for (auto &[rank, u_]: u) {
        ...
        MPIOperation<std::decay_t<T>> AllReduce(T data, U ...u) {
            logDebug(logName, std::format("{} all reduce data of type {}", rankID, typeid(T).name()));
        ...
        MPIOperation<void> AllReduce(MPIFakePacket p, U ...u) {
            logDebug(logName, std::format("{} all reduce fake data of type {}, fake parameters: {}", rankID, typeid(T).name(), to_string(u...)));
            MPIRankIDType root = co_await Elect(voteGenerator(*randomEngine));
        ...
        MPIOperation<void> RingAllReduce(MPIFakePacket p, std::size_t size, U ...u) {
            logDebug(logName, std::format("{} ring all reduce fake data of type {}, size: {}, fake parameters: {}", rankID, typeid(T).name(), size, to_string(u...)));
        ...
        MPIOperation<std::unordered_map<MPIRankIDType, R>> AllToAll(const std::unordered_map<MPIRankIDType, S> &data) {
            logDebug(logName, std::format("{} all to all data of type S: {} and type R: {}", rankID, typeid(S).name(), typeid(R).name()));
        ...
        MPIOperation<std::unordered_map<MPIRankIDType, T>> AllToAll(const std::unordered_map<MPIRankIDType, T> &data) {
            logDebug(logName, std::format("{} all to all data of type {}", rankID, typeid(T).name()));
        ...
        MPIOperation<void> AllToAll(MPIFakePacket p, const std::unordered_map<MPIRankIDType, std::tuple<US...>> &uS, const std::unordered_map<MPIRankIDType, std::tuple<UR...>> &uR) {
            logDebug(logName, std::format("{} all to all fake data of type S: {} and type R: {}, fake parameters omitted", rankID, typeid(S).name(), typeid(R).name()));
            std::vector<MPIOperation<void>> operations;
            for (auto &[rank, u]: uS) {
        ...
        MPIOperation<void> AllToAll(MPIFakePacket p, const std::tuple<US...> &uS, const std::tuple<UR...> &uR) {
            logDebug(logName, std::format("{} all to all fake data of type S: {} and type R: {}, fake parameters S: {}, fake parameters R: {}", rankID, typeid(S).name(), typeid(R).name(), to_string(uS), to_string(uR)));
        ...
        MPIOperation<void> AllToAll(MPIFakePacket p, const std::unordered_map<MPIRankIDType, std::tuple<U...>> &uS, const std::unordered_map<MPIRankIDType, std::tuple<U...>> &uR) {
            logDebug(logName, std::format("{} all to all fake data of type {}", rankID, typeid(T).name()));
        ...
        MPIOperation<void> AllToAll(MPIFakePacket p, const std::tuple<U...> &uS, const std::tuple<U...> &uR) {
            logDebug(logName, std::format("{} all to all fake data of type {}, fake parameters S: {}, fake parameters R: {}", rankID, typeid(T).name(), to_string(uS), to_string(uR)));
